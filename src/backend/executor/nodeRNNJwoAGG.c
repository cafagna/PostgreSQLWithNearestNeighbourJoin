...
#include "executor/nodeRNNJoinwoAGG.h"
...
int RNNJWoAggCompareSample(RNNJoinState *node, TupleTableSlot *outer,
	 TupleTableSlot *inner);
int RNNJWoAggCompareSampleInnerInner(RNNJoinState *node, TupleTableSlot *inner1,
	 TupleTableSlot *inner);
int
RNNJWoAggCompareGroup(RNNJoinState *node, TupleTableSlot *outer,
		TupleTableSlot *inner);
int
RNNJWoAggCompareTOuterInner(RNNJoinState *node, TupleTableSlot *outer,
		TupleTableSlot *inner);
int
RNNJWoAggCompareTInnerInner(RNNJoinState *node, TupleTableSlot *inner1,
		TupleTableSlot *inner2);
int
RNNJWoAggTimeDist(RNNJoinState *node, HeapTuple outer, TupleDesc outerDesc,
		TupleTableSlot *inner);

int RNNJWoAggTimeDistOutInn(RNNJoinState *node, TupleTableSlot *outer,
		TupleTableSlot *inner);

int RNNJWoAggCompareGroupBufInn(RNNJoinState *node, HeapTuple outer, TupleDesc outerDesc,
		TupleTableSlot *inner);

int RNNJWoAggCompareGroupInnInn(RNNJoinState *node, TupleTableSlot *inner1,
		TupleTableSlot *inner);
 ...
 */
TupleTableSlot *
ExecRNNJoinWoAgg(RNNJoinState *node) {
	List *joinqual;
	List *otherqual;
	bool qualResult;
	int compareResult;
	...
	for (;;) {
		...
		case EXEC_MJ_JOINTUPLES:
			...
			while (node->equiCont){
				...
				MJ_printf("ExecMergeJoin: returning equijoin tuple\n");
				...
				if (isDone != ExprEndResult) {
					...
					if ((node->outerequiBuffer->tail == node->equiCont)){
						node->equiCont = NULL;
						if (TupIsNull(node->mj_InnerTupleSlot) || RNNJWoAggCompareSampleInnerInner(node, node->mj_CurrInnerTupleSlot, node->mj_InnerTupleSlot) !=0 || RNNJWoAggCompareTInnerInner(node, node->mj_CurrInnerTupleSlot, node->mj_InnerTupleSlot) !=0 || RNNJWoAggCompareGroupInnInn(node, node->mj_CurrInnerTupleSlot, node->mj_InnerTupleSlot)!=0){
				...
				if (TupIsNull(node->mj_InnerTupleSlot)
						|| RNNJWoAggTimeDist(
								...
								> RNNJWoAggTimeDist(
										...
										lfirst(node->cont),
										node->mj_OuterTempTupleSlot->tts_tupleDescriptor,
										node->mj_CurrInnerTupleSlot) || ( RNNJWoAggCompareGroupBufInn(node, lfirst(node->cont), node->mj_OuterTempTupleSlot->tts_tupleDescriptor, node->mj_InnerTupleSlot)!=0 &&  RNNJWoAggCompareGroupBufInn(node, lfirst(node->cont), node->mj_OuterTempTupleSlot->tts_tupleDescriptor, node->mj_CurrInnerTupleSlot)==0)) {


					if (RNNJWoAggTimeDist(node, lfirst(node->cont),
							node->mj_OuterTempTupleSlot->tts_tupleDescriptor,
							node->mj_PrevInnerTupleSlot) == RNNJWoAggTimeDist(
							node, lfirst(node->cont),
							node->mj_OuterTempTupleSlot->tts_tupleDescriptor,
							node->mj_CurrInnerTupleSlot)
						&&
						RNNJWoAggCompareGroupInnInn(node, node->mj_CurrInnerTupleSlot, node->mj_PrevInnerTupleSlot)==0
					) {
						...
						while (node->aggCont){
							/*get values from aggSamples head and add to a new tuple*/

							oldContext = MemoryContextSwitchTo(
									node->mj_InnerTempTupleSlot->tts_mcxt);


							HeapTuple t = heap_copytuple(lfirst(node->aggCont));
							ExecStoreTuple(t, node->mj_InnerTempTupleSlot,
									InvalidBuffer, true);
							MemoryContextSwitchTo(oldContext);
							...
							if ((node->aggSamples->tail == node->aggCont)){
								node->aggCont = NULL;
							}
							else{
								node->prev_aggCont=node->aggCont;
								node->aggCont = node->prev_aggCont->next;
							}
						...
						while (node->localCont) {
							/*get values from localAggSamples head and add to a new tuple*/

							oldContext = MemoryContextSwitchTo(
									node->mj_InnerTempTupleSlot->tts_mcxt);

							HeapTuple t = heap_copytuple(lfirst(node->localCont));
							ExecStoreTuple(t, node->mj_InnerTempTupleSlot,
									InvalidBuffer, true);
							MemoryContextSwitchTo(oldContext);
							...
							oldContext = MemoryContextSwitchTo(
									node->mj_OuterTempTupleSlot->tts_mcxt);
							HeapTuple temporary = heap_copytuple(
									lfirst(node->cont));
							ExecStoreTuple(temporary,
									node->mj_OuterTempTupleSlot, InvalidBuffer,
									true);
							MemoryContextSwitchTo(oldContext);
							econtext->ecxt_outertuple =
									node->mj_OuterTempTupleSlot;
							econtext->ecxt_innertuple =
									node->mj_InnerTempTupleSlot;
							MJ_printf("ExecMergeJoin: returning tuple\n");
							result = ExecProject(node->js.ps.ps_ProjInfo,
									&isDone);

							if ((node->localAggSamples->tail == node->localCont)) {
								node->localCont = NULL;
							} else {
								node->prev_localCont = node->localCont;
								node->localCont = node->prev_localCont->next;
							}
					...
					if (list_length(node->outernnBuffer) == 0){
						...
						node->aggCont=list_head(node->aggSamples);
						node->prev_aggCont=NULL;
						node->localCont=list_head(node->localAggSamples);
		...
		case EXEC_MJ_NEXTINNER:
			MJ_printf("ExecMergeJoin: EXEC_MJ_NEXTINNER_wo_Agg\n");
			...
			if (!TupIsNull(node->mj_InnerTupleSlot)) {
				...
				if (RNNJWoAggCompareTInnerInner(node, node->mj_CurrInnerTupleSlot,
						node->mj_InnerTupleSlot) != 0 || RNNJWoAggCompareGroupInnInn(node, node->mj_CurrInnerTupleSlot, node->mj_InnerTupleSlot)!=0 && !TupIsNull(node->mj_OuterTupleSlot)) {
					if (!TupIsNull(node->mj_OuterTupleSlot) && RNNJWoAggTimeDistOutInn(  //SBAGLIATO: CONFRONTARE DISTANZA!!!
							node, node->mj_OuterTupleSlot,
							node->mj_CurrInnerTupleSlot) == RNNJWoAggTimeDistOutInn(
							node, node->mj_OuterTupleSlot,
							node->mj_InnerTupleSlot) && RNNJWoAggCompareGroupInnInn(node, node->mj_InnerTupleSlot, node->mj_CurrInnerTupleSlot)==0) { // AGGIUNGERE QUI

						node->aggSamples = list_copy(node->localAggSamples);

					} else {

						list_free(node->aggSamples);
						node->aggSamples = NIL;
					}
					list_free(node->localAggSamples);
					node->localAggSamples=NIL;
					ExecCopySlot(node->mj_PrevInnerTupleSlot,
							node->mj_CurrInnerTupleSlot);
				...
				if (TupIsNull(node->mj_OuterTupleSlot) || (RNNJWoAggCompareGroup(node, node->mj_OuterTupleSlot, node->mj_CurrInnerTupleSlot)==0)) {

					HeapTuple  sampleTupleSlot;
					sampleTupleSlot = ExecCopySlotTuple(node->mj_CurrInnerTupleSlot);
                    node->aggSamples = lappend(node->aggSamples,sampleTupleSlot);
					node->localAggSamples = lappend(node->localAggSamples,
							sampleTupleSlot);
		...
		case EXEC_MJ_NEXTOUTER:
			...
			while (!TupIsNull(outerTupleSlot) && (TupIsNull(innerTupleSlot)
					|| ((RNNJWoAggCompareTOuterInner(node, outerTupleSlot,
							node->mj_CurrInnerTupleSlot) <= 0 && RNNJWoAggCompareGroup(node,
							outerTupleSlot, node->mj_CurrInnerTupleSlot) == 0 && RNNJWoAggCompareSample(node, node->mj_OuterTupleSlot,
									node->mj_CurrInnerTupleSlot) <= 0)
							|| RNNJWoAggCompareGroup(node, outerTupleSlot,
									innerTupleSlot) < 0
							|| (RNNJWoAggTimeDistOutInn(node, outerTupleSlot,
									node->mj_CurrInnerTupleSlot) < RNNJWoAggTimeDistOutInn(node, outerTupleSlot,
											node->mj_InnerTupleSlot) && RNNJWoAggCompareGroup(node,
				...
				if (RNNJWoAggCompareGroup(node, outerTupleSlot,
					...
					if(RNNJWoAggCompareSample(node, node->mj_OuterTupleSlot,
									node->mj_CurrInnerTupleSlot)==0){
						MJ_printf("ExecMergeJoin: EQUIJOIN MATCH adding to equibuff\n");
		...
		default:
			 ...
			 */
RNNJoinState *
ExecInitRNNJoinWoAgg(MergeJoin *node, EState *estate, int eflags) {
	...
	return mergestate;
		 ...
		 * ----------------------------------------------------------------
		 */
void ExecEndRNNJoinWoAgg(RNNJoinState *node) {
...
void ExecReScanRNNJoinWoAgg(RNNJoinState *node) {
	ExecClearTuple(node->mj_MarkedTupleSlot);
...
int RNNJWoAggCompareGroup(RNNJoinState *node, TupleTableSlot *outer,
		TupleTableSlot *inner) {
	ListCell *ele1, *ele2;
	int result = 0;
	bool nulleqnull = false;
...
int RNNJWoAggCompareTOuterInner(RNNJoinState *node, TupleTableSlot *outer,
		TupleTableSlot *inner) {
	int result = 0;
	bool nulleqnull = false;
	ExprContext *econtext = node->js.ps.ps_ExprContext;
...
int RNNJWoAggCompareTInnerInner(RNNJoinState *node, TupleTableSlot *inner1,
		TupleTableSlot *inner2) {
	int result = 0;
	bool nulleqnull = false;
	ExprContext *econtext = node->js.ps.ps_ExprContext;
...
int RNNJWoAggTimeDist(RNNJoinState *node, HeapTuple outer, TupleDesc outerDesc,
		TupleTableSlot *inner) {
