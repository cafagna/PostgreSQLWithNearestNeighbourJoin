...
%token <keyword> ABORT_P ABSOLUTE_P ACCESS ACTION ADD_P ADMIN AFTER AGGR
	AGGREGATE ALL ALSO ALTER ALWAYS ANALYSE ANALYZE AND ANY ARRAY AS ASC
	ASSERTION ASSIGNMENT ASYMMETRIC AT ATTRIBUTE AUTHORIZATION

	BACKTRACK BACKWARD BEFORE BEGIN_P BETWEEN BIGINT BINARY BIT
	BOOLEAN_P BOTH BY
	...
	EACH ELSE ENABLE_P ENCODING ENCRYPTED END_P ENUM_P EQUAL ESCAPE EVENT EXCEPT
	...
	NAME_P NAMES NATIONAL NATURAL NCHAR NEXT NN NO NONE
	...
	RESET RESTART RESTRICT RETURNING RETURNS REVOKE RIGHT RNNJ ROLE ROLLBACK
	ROW ROWS RULE
 ...
 */
%left		JOIN CROSS LEFT FULL RIGHT INNER_P NATURAL RNNJ
...
table_ref:	relation_expr opt_alias_clause
			...
			| '(' joined_table ')' alias_clause
				...
				|  table_ref RNNJ table_ref EQUAL ON name_list NN BY  name_list USING name name_list name 
				{
					...
					n->aggClause = list_make1(makeString($13));
					n->alias = NULL;
					n->quals = NULL;
					n->group_pos_inner = NIL;
					n->group_pos_outer=NIL;

				    n->sample_pos_inner=NIL; 
			  	  	n->sample_pos_outer=NIL; 
    				...
    				n->aggCheck=FALSE;
    				n->positions=list_make1(makeString($13));
					n->buffCheck=FALSE;
			...
			|  table_ref RNNJ table_ref EQUAL ON name_list USING name AGGR name 
				{
					JoinExpr *n = makeNode(JoinExpr);
					n->jointype = JOIN_LEFT;
					n->larg = $1;
					...
					n->equalOnClause = $6;
					n->nnByClause = NIL;
					n->usingClause = list_make1(makeString($8));
					n->aggClause = list_make1(makeString($10));
					n->alias = NULL;
					n->quals = NULL;
					n->group_pos_inner = NIL;
					n->group_pos_outer=NIL;
				...
				|  table_ref RNNJ table_ref EQUAL ON name_list NN BY  name_list USING name  
				{
					JoinExpr *n = makeNode(JoinExpr);
					n->jointype = JOIN_LEFT;
					n->larg = $1;
				...
				|  table_ref RNNJ table_ref EQUAL ON name_list USING name 
				{
					JoinExpr *n = makeNode(JoinExpr);
					n->jointype = JOIN_LEFT;
					n->larg = $1;
				...
				|  table_ref RNNJ table_ref EQUAL ON name_list NN BY  name_list USING name BACKTRACK
				{
					JoinExpr *n = makeNode(JoinExpr);
					n->jointype = JOIN_LEFT;
					n->larg = $1;
				...
				|  table_ref RNNJ table_ref EQUAL ON name_list USING name BACKTRACK
				{
					JoinExpr *n = makeNode(JoinExpr);
					n->jointype = JOIN_LEFT;
					n->larg = $1;
