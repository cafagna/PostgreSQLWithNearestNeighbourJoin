						...
						List **namespace)
{
	...
	else if (IsA(n, JoinExpr) && !((JoinExpr*)n)->equalOnClause)
	{
	...
	else if (IsA(n, JoinExpr) && ((JoinExpr*)n)->equalOnClause)  //RNNJ
	{
		...
		*res_colvars;
		...
		/*
				...
				sv_namespace_length = list_length(pstate->p_namespace);
				pstate->p_namespace = list_concat(pstate->p_namespace, l_relnamespace);
		...
		j->sample_pos_inner=NIL;
		j->sample_pos_outer=NIL;
		j->group_pos_inner=NIL;
		j->group_pos_outer=NIL;
		j->time_pos_inner=-1;
		j->time_pos_outer=-1;
		j->pos_M=-1;
		j->positions=NIL;
				...
				*r_colvar;
				...
				foreach(col, res_colnames)
				{
					...
					if (strcmp(res_colname, u_colname) == 0)
						ereport(ERROR,
								(errcode(ERRCODE_DUPLICATE_COLUMN),
										errmsg("column name \"%s\" appears more than once in NN BY clause",
												u_colname)));
				}
							...
							ereport(ERROR,
									(errcode(ERRCODE_AMBIGUOUS_COLUMN),
											errmsg("common column name \"%s\" appears more than once in left table",
													u_colname)));
						l_index = ndx;
						j->group_pos_outer=lappend_int(j->group_pos_outer,l_index+1);
				...
				if (l_index < 0)
					ereport(ERROR,
							(errcode(ERRCODE_UNDEFINED_COLUMN),
									errmsg("column \"%s\" specified in NN BY clause does not exist in left table",
											u_colname)));
							...
							ereport(ERROR,
									(errcode(ERRCODE_AMBIGUOUS_COLUMN),
											errmsg("common column name \"%s\" appears more than once in right table",
													u_colname)));
						r_index = ndx;
						j->group_pos_inner=lappend_int(j->group_pos_inner,r_index+1);
					}
					ndx++;
				}
				if (r_index < 0)
					ereport(ERROR,
							(errcode(ERRCODE_UNDEFINED_COLUMN),
									errmsg("column \"%s\" specified in NN BY clause does not exist in right table",
											u_colname)));
		...
		if(j->usingClause)
		{
			...
			char	   *u_colname = strVal(lfirst(list_head(j->usingClause)));
			ListCell   *col;
			int			ndx;
			int			l_index = -1;
			int			r_index = -1;
			...
			*r_colvar;
			...
			res_colnames = lappend(res_colnames, lfirst(list_head(j->usingClause)));
			res_colvars = lappend(res_colvars,
					buildMergedJoinVar(pstate,
							j->jointype,
							l_colvar,
		...
		if (j->equalOnClause)
		{
				...
				*r_colvar;
				...
				foreach(col, res_colnames)
				{
					...
					if (strcmp(res_colname, u_colname) == 0)
						ereport(ERROR,
								(errcode(ERRCODE_DUPLICATE_COLUMN),
										errmsg("column name \"%s\" appears more than once in EQUAL ON clause",
												u_colname)));
				}
							...
							ereport(ERROR,
									(errcode(ERRCODE_AMBIGUOUS_COLUMN),
											errmsg("common column name \"%s\" appears more than once in left table",
													u_colname)));
						l_index = ndx;
						j->sample_pos_outer=lappend_int(j->sample_pos_outer,l_index+1);
				...
				if (l_index < 0)
					ereport(ERROR,
							(errcode(ERRCODE_UNDEFINED_COLUMN),
									errmsg("column \"%s\" specified in EQUAL ON clause does not exist in left table",
											u_colname)));
							...
							ereport(ERROR,
									(errcode(ERRCODE_AMBIGUOUS_COLUMN),
											errmsg("common column name \"%s\" appears more than once in right table",
													u_colname)));
						r_index = ndx;
						j->sample_pos_inner=lappend_int(j->sample_pos_inner,r_index+1);
					}
					ndx++;
				}
				if (r_index < 0)
					ereport(ERROR,
							(errcode(ERRCODE_UNDEFINED_COLUMN),
									errmsg("column \"%s\" specified in EQUAL ON clause does not exist in right table",
											u_colname)));
			...
			if(j->aggCheck)
			{
				...
				foreach(r_iterator, r_colnames){
					char	   *u_colname = strVal(lfirst(r_iterator));
					ListCell   *col;
					...
					if (!inRes){
						j->positions=lappend_int(j->positions,r_index+1);
					}
					r_index++;
		...
		if(j->aggClause)
		{
			...
			char	   *u_colname = strVal(lfirst(list_head(j->aggClause)));
			ListCell   *col;
			int			ndx;
			int			l_index = -1;
			int			r_index = -1;
			...
			foreach(col, res_colnames)
			{
				...
				if (strcmp(res_colname, u_colname) == 0)
					ereport(ERROR,
							(errcode(ERRCODE_DUPLICATE_COLUMN),
									errmsg("column name \"%s\" appears more than once in AGG clause",
											u_colname)));
			}
			...
			if (r_index < 0)
				ereport(ERROR,
						(errcode(ERRCODE_UNDEFINED_COLUMN),
								errmsg("column \"%s\" specified in AGG clause does not exist in right table",
										u_colname)));

			r_colvar = list_nth(r_colvars, r_index);
			res_colvars = lappend(res_colvars, r_colvar);
			res_colnames = lappend(res_colnames, lfirst(list_head(j->aggClause)));
		...
		if (j->aggCheck){
			if (j->alias)
				...
				if (j->alias != NULL)
					...
					setNamespaceColumnVisibility(my_relnamespace, false);
