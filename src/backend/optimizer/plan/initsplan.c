					...
					List **postponed_qual_list)
{
	...
	else if (IsA(jtnode, JoinExpr))
	{
		...
		if(j->equalOnClause) /* check if it is nn join */
		{
			j->jointype = JOIN_RNNJ;
		}
		...
		if (j->jointype != JOIN_INNER)
		{
			...
			if(j->equalOnClause) /* check if it is nn join */
			{
				sjinfo->group_pos_inner=(List*)copyObject(j->group_pos_inner);
				sjinfo->group_pos_outer=(List*)copyObject(j->group_pos_outer);
				sjinfo->sample_pos_inner=(List*)copyObject(j->sample_pos_inner);
				sjinfo->sample_pos_outer=(List*)copyObject(j->sample_pos_outer);
				sjinfo->time_pos_inner=j->time_pos_inner;
				sjinfo->time_pos_outer=j->time_pos_outer;
				sjinfo->aggFuncs=(List*)copyObject(j->aggFuncs);
				sjinfo->pos_M=j->pos_M;
				sjinfo->aggCheck=j->aggCheck;
				sjinfo->positions=(List*)copyObject(j->positions);
				sjinfo->buffCheck=j->buffCheck;
