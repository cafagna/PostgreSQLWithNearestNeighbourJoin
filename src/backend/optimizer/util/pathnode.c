					  ...
					  List *innersortkeys)
{
	...
	if(sjinfo->jointype==JOIN_RNNJ)
		{
			pathnode->group_pos_inner=(List*)copyObject(sjinfo->group_pos_inner);
			pathnode->group_pos_outer=(List*)copyObject(sjinfo->group_pos_outer);
			pathnode->sample_pos_inner=(List*)copyObject(sjinfo->sample_pos_inner);
			pathnode->sample_pos_outer=(List*)copyObject(sjinfo->sample_pos_outer);
			pathnode->time_pos_inner=sjinfo->time_pos_inner;
			pathnode->time_pos_outer=sjinfo->time_pos_outer;
			pathnode->aggFuncs=(List*)copyObject(sjinfo->aggFuncs);
			pathnode->pos_M=sjinfo->pos_M;
			pathnode->aggCheck=sjinfo->aggCheck;
			pathnode->positions=(List*)copyObject(sjinfo->positions);
			pathnode->buffCheck=sjinfo->buffCheck;
